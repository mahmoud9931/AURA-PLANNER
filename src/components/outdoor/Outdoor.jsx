...
export default function Outdoor() {
    ...
    const fetchVenues = () => {
        setLoading(true);
        axios.get(`${API_BASE}/api/outdoors`)
            .then((res) => {
    ...
    const handleEdit = async (e) => {
        ...
        try {
            await axios.put(
                `${API_BASE}/api/outdoors/${selectedVenue._id}`,
                { ...formData, type: "outdoor" },
                getAuthHeaders()
            );
    ...
    const handleDelete = async (venueId, venueName) => {
        ...
        try {
            await axios.delete(`${API_BASE}/api/outdoors/${venueId}`, getAuthHeaders());
            fetchVenues();
        } catch (err) {
        ...
        <>
            ...
            <section className="venues-section">
                <div className="venues-title-row">
                    <h1 className="venues-title">OUTDOOR</h1>
                    {isAdmin && (
                        ...
                        <div className="venue-card" key={venue._id}>
                            ...
                            <div className="venue-body">
                                ...
                                <div className="venue-footer">
                                    ...
                                    <Link to={`/venues/outdoor/${venue._id}`} className="venue-btn">Book Now</Link>
                                </div>
                                {isAdmin && (
                ...
                <div className="modal-overlay" onClick={() => setShowAddModal(false)}>
                    <div className="modal-box" onClick={(e) => e.stopPropagation()}>
                        <div className="modal-header">
                            <h2 className="modal-title">➕ Add New Outdoor Venue</h2>
                        ...
                        <form className="modal-form" onSubmit={handleAdd}>
                            ...
                            <div className="modal-actions">
                                ...
                                <button type="submit" className="modal-btn modal-btn-submit" disabled={submitting}>{submitting ? "Saving…" : "Add Venue"}</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    return (
        <>
            <div className="form-group">
                ...
                <input className="form-input" type="text" name="name" value={formData.name} onChange={handleChange} placeholder="e.g. Garden Paradise" required />
            </div>
            <div className="form-group">
                <label className="form-label">Description</label>
                <textarea className="form-input form-textarea" name="description" value={formData.description} onChange={handleChange} placeholder="Brief description of the venue…" rows={3} />
            </div>
