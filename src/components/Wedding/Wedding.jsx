...
export default function Wedding() {
    ...
    const fetchVenues = () => {
        setLoading(true);
        axios.get(`${API_BASE}/api/weddings`)
            .then((res) => {
    ...
    const handleAdd = async (e) => {
        ...
        setSubmitting(true);
        try {
            await axios.post(`${API_BASE}/api/weddings`, { ...formData, type: "wedding" }, getAuthHeaders());
    ...
    const handleEdit = async (e) => {
        ...
        setSubmitting(true);
        try {
            await axios.put(`${API_BASE}/api/weddings/${selectedVenue._id}`, { ...formData, type: "wedding" }, getAuthHeaders());
    ...
    const handleDelete = async (venueId, venueName) => {
        ...
        try {
            await axios.delete(`${API_BASE}/api/weddings/${venueId}`, getAuthHeaders());
            fetchVenues();
        } catch (err) {
                        ...
                        <div className="venue-card" key={venue._id}>
                            ...
                            <div className="venue-body">
                                ...
                                <div className="venue-footer">
                                    ...
                                    <Link to={`/venues/weddings/${venue._id}`} className="venue-btn">Book Now</Link>
                                </div>
                                {isAdmin && (
                ...
                <div className="modal-overlay" onClick={() => setShowAddModal(false)}>
                    <div className="modal-box" onClick={(e) => e.stopPropagation()}>
                        <div className="modal-header">
                            <h2 className="modal-title">➕ Add Wedding &amp; Engagement Idea</h2>
                            <button className="modal-close" onClick={() => setShowAddModal(false)}>✕</button>
                        </div>
                        <form className="modal-form" onSubmit={handleAdd}>
                            <VenueFormFields formData={formData} handleChange={handleChange} placeholder="e.g. Romantic Garden Setup" />
...
function VenueFormFields({ formData, handleChange, placeholder = "e.g. Crystal Hall" }) {
    return (
        <>
            <div className="form-group">
                <label className="form-label">Name <span className="required">*</span></label>
                <input className="form-input" type="text" name="name" value={formData.name} onChange={handleChange} placeholder={placeholder} required />
            </div>
            <div className="form-group">
